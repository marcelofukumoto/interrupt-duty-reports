...
//
// The row is built around the one number somebody scanning for work needs - how many items are
// owed a move - with the rest of the counts kept quiet beside it. Before, every count was the
// same weight and the same grey, so a day with nine things to do looked exactly like a day with
// none.
...
const emit = defineEmits<{
  ...
  (e: 'confirm-delete', id: string | null): void;
...
<template>
  ...
  >
    ...
    >
      ...
      <RunProgress
        v-else-if="meta.status === 'running'"
        :phase="progress?.phase || 'starting'"
        :output="progress?.output || ''"
        :elapsed="elapsedLabel(meta)"
      />
    ...
    <div class="row__side">
      ...
      <template v-if="confirming">
        <button type="button" class="btn btn-sm role-secondary" :disabled="deleting" @click.stop="emit('confirm-delete', null)">
          Cancel
        </button>
        <button
          type="button"
      ...
      <button
        ...
        @click.stop="emit('confirm-delete', meta.id)"
      >
