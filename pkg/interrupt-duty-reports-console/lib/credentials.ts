// The two credentials a report needs, stored the way Extension Studio stores its GitHub token.
//
...
/**
 * Extension Studio's, borrowed when it has one.
 *
 * The same key name in the same kind of Secret, because it is the same credential: an account's
 * GitHub token, reused by everything that publishes on their behalf. Asking somebody to paste it
 * twice into one Rancher would be asking them to keep two copies of one secret in step.
 */
export const STUDIO_NAMESPACE = 'extension-studio';
export const STUDIO_SECRET = 'settings';

/** The key names are the Secret's, so `kubectl get secret settings -o yaml` reads plainly. */
export const GH_TOKEN_KEY = 'gh_token';
export const JIRA_PAT_KEY = 'jira_pat';

const GH_ANNOTATION = 'interrupt-duty.rancher.io/gh-token';
const JIRA_ANNOTATION = 'interrupt-duty.rancher.io/jira-pat';
...
export async function readCredentialStatus(): Promise<CredentialStatus> {
  const [ours, studio] = await Promise.all([
    secretMetadata(CRED_NAMESPACE, CRED_SECRET),
  ...
  return {
    ...
    jira:       ourAnnotations[JIRA_ANNOTATION] === 'set' || ourKeys.includes(JIRA_PAT_KEY),
    unreadable: false,
  };
}
...
export async function saveCredentials(changes: CredentialChanges): Promise<void> {
  ...
  if (changes.jiraPat !== undefined) {
    ...
    annotations[JIRA_ANNOTATION] = changes.jiraPat === '' ? null : 'set';
  }
...
/** Whether a run can start: both credentials resolvable from somewhere. */
export function credentialsReady(status: CredentialStatus): boolean {
  return status.gh !== 'none' && status.jira;
}
